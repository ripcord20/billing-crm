...
async function isolirCustomer(customerId, triggerBy = 'admin', adminUserId = null) {
  ...
  const hasPppoeUser = !!(cust.pppoe_username && String(cust.pppoe_username).trim());
  let radiusEnabled = radiusPrep;
  if (!radiusEnabled && hasPppoeUser) {
    try { radiusEnabled = await require('./RadiusProvisionService').isEnabled(cust.tenant_id); } catch (_) {}
  }

  if (!cust.mikrotik_id) {
    if (radiusEnabled && hasPppoeUser) radiusOnly = true;
    else throw new Error('MikroTik device belum dipilih');
  }
  ...
  try {
    ...
    try {
      if (method === 'hotspot_binding') {
        ...
        // ── Method PPPoE: RADIUS (Auth-Type Reject) bila aktif, else switch profile ──
        let usedRadius = false;
        try {
          const RadiusProv = require('./RadiusProvisionService');
          if (await RadiusProv.hasAccount(cust.id) || await RadiusProv.isEnabled(cust.tenant_id)) {
            const r = await RadiusProv.isolir(cust);
            if (r.success) {
              usedRadius = true;
              method = 'radius';
              let kicked = 0;
              try {
                ...
                kicked = await IsolirPPPoE.kickActiveSessions(api, pppoeUser);
              } catch (_) {}
              methodDetail = `RADIUS Reject${kicked ? `, ${kicked} sesi di-kick` : ''}`;
            }
          }
        } catch (re) {
          console.warn('[Isolir] RADIUS: ' + re.message);
        }
        if (!usedRadius) {
          const IsolirPPPoE = require('./IsolirPPPoE');
...
async function restoreCustomer(customerId, triggerBy = 'admin', adminUserId = null) {
  ...
  if (radiusOnly) {
    ...
    await logIsolir({
      customer_id: customerId, device_id: cust.mikrotik_id || null,
      static_ip: cust.static_ip || null, pppoe_username: cust.pppoe_username || null,
      isolir_method: 'radius', action: 'restore', trigger_by: triggerBy,
      triggered_by_user: adminUserId, success: true, error_msg: null
    });
    ...
    try {
      if (method === 'hotspot_binding') {
        ...
        try {
          const RadiusProv = require('./RadiusProvisionService');
          if (await RadiusProv.hasAccount(cust.id) || await RadiusProv.isEnabled(cust.tenant_id)) {
            const r = await RadiusProv.restore(cust);
            if (r.success) {
              usedRadius = true;
              method = 'radius';
              let kicked = 0;
              ...
              methodDetail = `RADIUS restore${kicked ? `, ${kicked} sesi di-kick` : ''}`;
            }
          }
        } catch (re) {
          console.warn('[Restore] RADIUS: ' + re.message);
        }
        if (!usedRadius) {
          const IsolirPPPoE = require('./IsolirPPPoE');
