...
async function attachManagedDevice(row, hint = {}) {
  if (!row) return null;
  const device = await findManagedDevice(Device, {
    nasname: hint.nasname || row.nasname,
    shortname: hint.shortname || row.shortname,
    device_id: hint.device_id || row.device_id
  });
  if (device && row.device_id !== device.id) {
...
class NasController {
  async index(req, res) {
    try {
      const orphans = await NasDevice.findAll({ where: { device_id: null } });
      for (const row of orphans) {
        await attachManagedDevice(row);
      }
  ...
  async create(req, res) {
    try {
      const b = req.body || {};
      const nasname = normalize(b.nasname);
      const shortname = normalize(b.shortname) || nasname;
      if (!nasname || !b.secret) {
      ...
      const existing = await NasDevice.findOne({ where: { nasname } });
      if (existing) {
        const patch = {};
        if (device && existing.device_id !== device.id) patch.device_id = device.id;
        if (shortname && existing.shortname !== shortname) patch.shortname = shortname;
        if (b.conn_mode && existing.conn_mode !== connMode) {
          patch.conn_mode = connMode;
          patch.vpn_type = connMode === 'vpn' ? (b.vpn_type || existing.vpn_type || 'wireguard') : existing.vpn_type;
        }
        if (b.secret && b.secret !== '********') patch.secret = b.secret;
        if (Object.keys(patch).length) await existing.update(patch);
        const sync = await pushNas(existing);
        return res.json({
          success: true,
          data: existing,
          radius_sync: sync,
          reused: true,
          device_linked: !!(device || existing.device_id)
        });
      }
      const row = await NasDevice.create({
        tenant_id: getTenantId() || b.tenant_id || null,
        radius_server_id: b.radius_server_id || null,
        device_id: device ? device.id : (b.device_id || null),
        nasname,
        shortname,
      ...
      res.status(201).json({ success: true, data: row, radius_sync: sync, device_linked: !!device });
    } catch (e) {
  ...
  async update(req, res) {
    try {
      ...
      await row.update(patch);
      await attachManagedDevice(row, {
        nasname: patch.nasname || row.nasname,
        shortname: patch.shortname || row.shortname,
