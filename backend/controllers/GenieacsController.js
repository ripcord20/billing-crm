...
// ============================================================
// Behavior:
//   - GenieACS receive task & trigger Connection Request ke ONT
//   - Kalau ONT reachable (port 7547 dari ACS) → task execute langsung
//     (HTTP 200, executed=true)
//   - Kalau ONT di belakang NAT/firewall → Connection Request timeout, task
//     di-queue di ACS (HTTP 202, executed=false). Akan execute saat ONT
//     periodic inform berikutnya — bisa hitungan detik sampai menit tergantung
//     PeriodicInformInterval.
//
// Response include:
//   - executed: true   → ONT reboot sekarang (2-10 detik)
//   - executed: false  → task pending, ETA berapa menit
// ============================================================
exports.rebootDevice = async (req, res) => {
  try {
    ...
    // Kalau task gagal di-create sama sekali, return error
    if (!result.success) {
      return res.json(result);
    }

    // Kalau task queued (Connection Request fail), ambil ETA
    let eta = null;
    if (result.queued) {
      eta = await genieacs.getInformEta(deviceId);
    }
...
// ============================================================
// GET /api/genieacs/devices/:id/rx-history — RX Power time-series dari
// tabel ont_signal_history (riwayat sebenarnya), bukan single-row ont_devices.
// ============================================================
exports.getRxHistory = async (req, res) => {
  try {
    ...
    // 1) Map device GenieACS → baris ont_devices (untuk dapat ont_device_id).
    //    device_id = ID GenieACS; fallback ke serial_number bila perlu.
    const ont = await OntDevice.findOne({
      where: { [Op.or]: [{ device_id: deviceId }, { serial_number: deviceId }] },
      attributes: ['id', 'signal_strength', 'last_synced', 'last_inform'],
    });

    if (!ont) {
      // Tidak ada di DB → coba ambil current dari GenieACS sebagai 1 titik.
      const result = await genieacs.getDevice(deviceId, 'VirtualParameters.RXPower,_lastInform').catch(() => null);
      const current = result?.data?.VirtualParameters?.RXPower?._value;
      return res.json({
        success: true,
        data: current != null ? [{ time: new Date(), value: parseFloat(current) }] : [],
        message: 'Perangkat belum tersinkron ke database'
      });
    }

    // 2) Ambil time-series dari ont_signal_history dalam rentang jam terpilih.
    ...
    const rows = await OntSignalHistory.findAll({
      ...
      attributes: ['rx_power', 'recorded_at'],
    ...
    if (rows && rows.length) {
      const data = rows.map(r => ({
        time:  r.recorded_at,
        value: parseFloat(r.rx_power),
      }));
      return res.json({ success: true, data });
    }

    // 3) Belum ada histori di rentang ini → fallback 1 titik dari nilai terkini
    //    pada ont_devices (supaya chart tetap menampilkan posisi sinyal saat ini).
    if (ont.signal_strength != null) {
      return res.json({
        success: true,
        data: [{ time: ont.last_synced || ont.last_inform || new Date(), value: parseFloat(ont.signal_strength) }],
        message: 'Belum ada histori pada rentang ini — menampilkan nilai terkini'
      });
    }

    return res.json({ success: true, data: [], message: 'Belum ada histori sinyal' });
  } catch(err) {
