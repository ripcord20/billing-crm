...
// 7) TenantSignupService: password acak & daftar paket default
const pw1 = TenantSignupService.genPassword();
const pw2 = TenantSignupService.genPassword();
assert.strictEqual(pw1.length, 10);
assert.notStrictEqual(pw1, pw2);
assert.ok(Array.isArray(TenantSignupService.DEFAULT_PLANS) && TenantSignupService.DEFAULT_PLANS.length >= 1);
assert.ok(TenantSignupService.DEFAULT_PLANS.every(p => p.code && p.name && typeof p.price === 'number'));
