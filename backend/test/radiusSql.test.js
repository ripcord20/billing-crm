...
const { extractPppoeFields, stripPppoeFields, describeMikrotikPppError } = require('../services/PppoeAccountService');
...
const perm = describeMikrotikPppError(new Error('not enough permissions (9)'));
assert.ok(/RADIUS/.test(perm));
assert.ok(!/\(9\)/.test(perm) || /izin/.test(perm));

const stripped = stripPppoeFields({
  name: 'Budi',
  radius_password: 'x',
  pppoe_password: 'keep-me',
  create_pppoe: true,
  pppoe_profile: '20Mbps',
  phone: '0812',
  device_id: 9
...
assert.strictEqual(stripped.pppoe_password, 'keep-me');
